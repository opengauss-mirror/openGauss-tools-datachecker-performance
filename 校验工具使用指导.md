...
data:
  check:
    ...
    create-repair-sql: false # 默认关闭修复语句输出
rules:
...
spring:
  ...
  datasource:
    ...
    url: jdbc:opengauss://127.0.0.1:5432/test?useSSL=false&useUnicode=true&characterEncoding=utf-8&serverTimezone=UTC&loggerLevel=OFF
    # driver-class-name: oracle.jdbc.OracleDriver
    # url: jdbc:oracle:thin:@127.0.0.1:1521/TEST
...
`run.sh` 是一个完整的 datachecker 应用管理脚本，用于统一启动、停止和检查所有相关进程。

**脚本功能**
- 自动启动/停止/检查三个组件：source 实例、sink 实例、check 实例
- 支持自定义工作空间目录
- 自动创建必要的目录结构
- 统一管理 JVM 参数
- 可选的 Kafka 连接配置
...
# 启动应用（默认工作空间，Kafka 禁用）
sh run.sh start

# 启动应用（指定工作空间）
sh run.sh start /path/to/workspace

# 启动应用（启用 Kafka 配置）
sh run.sh start --kafka-bootstrap-servers <kafka_bootstrap_servers> --kafka-topic <kafka_topic>

# 启动应用（指定工作空间和 Kafka 配置）
sh run.sh start /path/to/workspace --kafka-bootstrap-servers <kafka_bootstrap_servers> --kafka-topic <kafka_topic>

# 启动应用（自定义 Kafka 键前缀）
sh run.sh start --kafka-bootstrap-servers <kafka_bootstrap_servers> --kafka-topic <kafka_topic> --kafka-key-prefix my_check

# 启动应用（指定工作空间和 Kafka 配置 自定义 Kafka 键前缀）
sh run.sh start /path/to/workspace --kafka-bootstrap-servers <kafka_bootstrap_servers> --kafka-topic <kafka_topic> --kafka-key-prefix my_check

# 启动应用（同时指定 JVM 参数和 Kafka 配置）
sh run.sh start --kafka-bootstrap-servers <kafka_bootstrap_servers> --kafka-topic <kafka_topic> --kafka-key-prefix my_check -Xmx2G -Xms2G

# 停止应用
sh run.sh stop

# 停止指定工作空间的应用
sh run.sh stop /path/to/workspace

# 检查应用状态
sh run.sh status

# 检查指定工作空间的应用状态
sh run.sh status /path/to/workspace
```

**命令格式**

```
sh run.sh COMMAND [WORKSPACE_DIR] [OPTIONS] [JVM_PARAMS]
```
...
| 参数 | 说明 | 默认值 |
|------|------|--------|
| `COMMAND` | 命令：`start`、`stop`、`status`、`help` | - |
| `WORKSPACE_DIR` | 工作空间目录（可选） | 脚本所在目录 |
| `--kafka-bootstrap-servers <servers>` | Kafka 引导服务器地址（可选） | 空（禁用） |
| `--kafka-topic <topic>` | Kafka 主题（可选） | 空（禁用） |
| `--kafka-key-prefix <prefix>` | Kafka 键前缀（可选） | `data_check` |
| `JVM_PARAMS` | JVM 参数（可选） | `-Xmx1G -Xms1G -XX:+UseG1GC -XX:MaxGCPauseMillis=100 -XX:+ParallelRefProcEnabled -XX:+HeapDumpOnOutOfMemoryError` |

**Kafka 配置说明**
- 当同时指定 `--kafka-bootstrap-servers` 和 `--kafka-topic` 时，脚本会自动在启动命令中添加 Kafka 相关参数
- 当未指定 Kafka 参数时，脚本会自动取消 Kafka 相关参数
- Kafka 键会自动生成为：`{prefix}_source`、`{prefix}_sink`、`{prefix}_check`
...
失败修复报告为纯文本修复SQL内容, 修复报告默认不输出。如需输出可通过如下配置打开 

application.yml
````yaml
data:
  check:
    create-repair-sql: true # true 打开修复语句输出
````
